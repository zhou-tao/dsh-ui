...
function qrSvg(text, opts = {}) {
  const qr = import_qrcode.default.create(text, {
  ...
  return '<svg xmlns="http://www.w3.org/2000/svg" width="' + total + '" height="' + total + '" viewBox="0 0 ' + total + " " + total + '"><rect width="100%" height="100%" fill="' + light + '"/><path d="' + rects.join("") + '" fill="' + dark + '"/></svg>';
}
...
var tunnelProc = null;
var lastClientAt = null;
function isLoopback(addr) {
  return addr === "127.0.0.1" || addr === "::1" || addr === "::ffff:127.0.0.1";
}
function noteClient(req) {
  const remote = req.socket.remoteAddress ?? "";
  const proxied = typeof req.headers["cf-connecting-ip"] === "string" || typeof req.headers["x-forwarded-for"] === "string" || typeof req.headers["x-forwarded-proto"] === "string";
  if (proxied || !isLoopback(remote)) {
    lastClientAt = Date.now();
  }
}
function connectedNow() {
  return lastClientAt !== null && Date.now() - lastClientAt < 18e4;
}
function lanIp() {
...
var HISTORY_NOISE_TYPES = /* @__PURE__ */ new Set([
  "assistant/chunk",
  "step/start",
  "request/header",
  "request/context",
...
var TRIM_TOOL_ARG = 240;
var TRIM_TOOL_RESULT = 400;
function textBlocksOnly(blocks) {
  return blocks.filter((b) => b !== null && typeof b === "object" && b.type === "text");
}
...
function slimHistoryResponse(text) {
  try {
    ...
    if (env?.type === "server-response" && env.result?.ok === true && Array.isArray(env.result.value?.events)) {
      ...
      for (const it of env.result.value.events) {
        ...
        if (HISTORY_NOISE_TYPES.has(t)) continue;
        delete it.view;
        slimEvent(it.event);
        kept.push(it);
      }
...
async function handleStatic(req, res, pathname, url) {
  noteClient(req);
